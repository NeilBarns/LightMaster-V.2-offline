document.addEventListener('DOMContentLoaded', function () {
     ...
     */
    const btnUpdateIncrementStatusList = document.querySelectorAll('.btnUpdateIncrementStatus');
     ...
     */
    const btnCancelNodeExchangeList = document.querySelectorAll('.btnCancelNodeExchange');
     ...
     */
    const btnSaveRemainingTime = document.querySelectorAll('.btnSaveRemainingTime');

    /**
     * Device Rate and Usage Chart
     */
    const btnMonthlyDeviceLevel = document.querySelectorAll('.btnMonthlyDeviceLevel');
    const btnDailyDeviceLevel = document.querySelectorAll('.btnDailyDeviceLevel');
    const canvasDeviceLevelRateAndUsageList = document.querySelectorAll('#canvasDeviceLevelRateAndUsage');

    /**
     * Device Overview Report
     */
    const divDeviceLevelOverview = document.querySelectorAll('.divDeviceLevelOverview');

    /**
     * Device Detailed Report
     */
    const divDeviceLevelDetailed = document.querySelectorAll('.divDeviceLevelDetailed');

    /**
     * Overview report button
     */
    const btnDeviceLevelOverviewList = document.querySelectorAll('.btnDeviceLevelOverview');

    /**
     * Detailed report button
     */
    const btnDeviceLevelDetailedList = document.querySelectorAll('.btnDeviceLevelDetailed');
                            ...
                            UpdateDeviceName(deviceId, newDeviceName)
                                .then((result) => {
                                    if (!result.success) {
                                        ...
                                        if (lblDeviceName) {
                                            lblDeviceName.textContent = newDeviceName;
                                        }
                                    }
                                    HideLoading();
                                });
                ...
                item.addEventListener('click', function() {
                    ...
                    if (txtBaseTime && txtBaseRate)
                    {
                        ...
                        if (newBaseTime > 0 && newBaseRate > 0)
                        {
                            ...
                            UpdateDeviceBaseRateTime(deviceId, newBaseTime, newBaseRate)
                                .then((result) => {
                                    if (!result.success) {
                                        showToast(result.message, 'error');
                                    } else {
                        ...
                        {
                            showToast('Please enter a valid base time and rate greater than 0.', 'error');
                        }
                    }
        ...
        btnFreeTimeLimitList.forEach(item => {
            if (!item.classList.contains('event-attached')) {
                item.addEventListener('click', function() {
                    ...
                    if (txtFreeTimeLimit)
                    {
                        const newFreeLightTimeLimitValue = txtFreeTimeLimit.value;  

                        if (newFreeLightTimeLimitValue > 0)
                        {
                            ShowLoading();

                            UpdateFreeLightTimeLimit(deviceId, newFreeLightTimeLimitValue)
                                .then((result) => {
                                    if (!result.success) {
                                        showToast(result.message, 'error');
                                    } else {
                        ...
                        {
                            showToast('Please enter a valid free time limit greater than 0.', 'error');
                        }
                    }
                ...
                item.addEventListener('click', function() {
                    ...
                    if (txtOpenTime && txtOpenRate)
                    {
                        ...
                        if (newOpenTime > 0 && newOpenRate > 0)
                        {
                            ...
                            UpdateDeviceOpenRateTime(deviceId, newOpenTime, newOpenRate)
                                .then((result) => {
                                    if (!result.success) {
                                        showToast(result.message, 'error');
                                    } else {
                        ...
                        {
                            showToast('Please enter a valid open time and rate greater than 0.', 'error');
                        }
                    }
        ...
        btnUpdateIncrementStatusList.forEach(item => {
            if (!item.classList.contains('event-attached')) {
                item.addEventListener('click', function() {
                    ...
                    UpdateDeviceIncrementStatus(deviceId, deviceTimeID, newStatus)
                        .then((result) => {
                            if (!result.success) {
                                showToast(result.message, 'error');
                            } else {
                                ...
                                const bannerIncrementStatus = getBannerIncrementStatus(deviceTimeID);
                                const imageIncrementStatus = getImageIncrementStatus(deviceTimeID);
    
                                if (bannerIncrementStatus) {
                                    if (newStatus === '0') {
                                        bannerIncrementStatus.classList.add('!block');
                                        bannerIncrementStatus.classList.remove('!hidden');
                                    } else {
                                        bannerIncrementStatus.classList.remove('!block');
                                        bannerIncrementStatus.classList.add('!hidden');
                                    }
                                }
    
                                if (imageIncrementStatus) {
                                    if (newStatus === '0') {
                                        imageIncrementStatus.src = imageIncrementStatus.dataset.enableUrl;
                                    } else {
                                        imageIncrementStatus.src = imageIncrementStatus.dataset.disableUrl;
                                    }
                                }
                            }
                            HideLoading();
                ...
                item.addEventListener('click', function() {
                    ...
                    if (txtRemainingTime)
                    {
                        ...
                        if (newRemainingTime >= 0)
                        {
                            ...
                            UpdateDeviceRemainingTimeNotification(deviceId, newRemainingTime)
                                .then((result) => {
                                    if (!result.success) {
                                        showToast(result.message, 'error');
                                    } else {
    ...
    if (canvasDeviceLevelRateAndUsageList)
    {
        canvasDeviceLevelRateAndUsageList.forEach((item) => {
            if (item)
            {
                ...
                new DeviceRateAndUsageReportHandler(deviceId, item, VIEWTYPE.Monthly, DATA_INCLUSION_TYPE.BYDEVICE).DoSession();
            }
        });
    }
                ...
                item.addEventListener('click', function() {
                    ...
                    if (canvasDeviceLevelRateAndUsageList)
                    {
                        canvasDeviceLevelRateAndUsageList.forEach((item) => {
                            ...
                            if (item)
                            {
                                new DeviceRateAndUsageReportHandler(deviceId, item, VIEWTYPE.Daily, DATA_INCLUSION_TYPE.BYDEVICE).DoSession();
                            }
    ...
    if (divDeviceLevelOverview)
    {
        divDeviceLevelOverview.forEach((item) => {
            if (item)
            {
                ...
                const grdDeviceLevelOverview = getGrdDeviceLevelOverview(deviceId);

                if (grdDeviceLevelOverview)
                {
                    new DeviceOverviewTimeTransactionsReportHandler(deviceArray, grdDeviceLevelOverview).DoSession();
                }
            }
        });
    }
    ...
    if (divDeviceLevelDetailed) 
    {
        divDeviceLevelDetailed.forEach((item) => {
            if (item) {
                ...
                const grdDeviceLevelDetailed = getGrdDeviceLevelDetailed(deviceId);
    
                if (grdDeviceLevelDetailed) {
                    new DeviceDetailedTimeTransactionsReportHandler(deviceArray, grdDeviceLevelDetailed).DoSession();
                }
            }
        });
    }
            ...
            if (!item.classList.contains('event-attached')) {
                item.addEventListener('click', function() {
                    ...
                    if (grdDeviceLevelOverview)
                    {
                        ...
                        const divDeviceLevelDetailed = getDivDeviceLevelDetailed(deviceId);
                        const divDeviceLevelOverview = getDivDeviceLevelOverview(deviceId);
                        const btnDeviceLevelDetailed = getBtnDeviceLevelDetailed(deviceId);
                        ...
                        {
                            btnDeviceLevelDetailed.classList.remove("bg-blue-500", "text-white");
                            btnDeviceLevelDetailed.classList.add("bg-gray-200", "text-gray-800");
                        }
                    }
                });
                item.classList.add('event-attached');
            ...
            if (!item.classList.contains('event-attached')) {
                item.addEventListener('click', function() {
                    ...
                    if (grdDeviceLevelDetailed) {
                        ...
                        const divDeviceLevelDetailed = getDivDeviceLevelDetailed(deviceId);
                        const divDeviceLevelOverview = getDivDeviceLevelOverview(deviceId);
                        const btnDeviceLevelOverview = getBtnDeviceLevelOverview(deviceId);
                        ...
                        {
                            btnDeviceLevelOverview.classList.remove("bg-blue-500", "text-white");
                            btnDeviceLevelOverview.classList.add("bg-gray-200", "text-gray-800");
                        }
                    }
