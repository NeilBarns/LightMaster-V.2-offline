document.addEventListener("DOMContentLoaded", function () {
    ...
    const singleExtendButtonList = document.querySelectorAll(
        ".extend-time-single-button"
    );
        ...
        networkControlsList.forEach((item) => {
            if (!item.classList.contains("event-attached")) {
                item.addEventListener("click", function () {
                    ...
                    setTimeout(() => {
                        GetDeviceHeartbeat(deviceId).then((result) => {
        ...
        resumeButtonList.forEach((item) => {
            if (!item.classList.contains("event-attached")) {
                item.addEventListener("click", function () {
                    ...
                    ResumeDeviceTime(deviceId).then((result) => {
                        if (!result.success) {
                            showToast(
                                result.message ||
                                    "Possible network error occurred. Please see logs for more info.",
        ...
        pauseButtonList.forEach((item) => {
            if (!item.classList.contains("event-attached")) {
                item.addEventListener("click", function () {
                    ...
                    PauseDeviceTime(deviceId).then((result) => {
                        if (!result.success) {
                            showToast(
                                result.message ||
                                    "Possible network error occurred. Please see logs for more info.",
    ...
    if (extendButtonList) {
        extendButtonList.forEach((item) => {
            if (!item.classList.contains("event-attached")) {
                item.addEventListener("click", function () {
                    const deviceId =
                        this.closest(".dropdown").getAttribute("data-id");
                    const extendTime = this.getAttribute("data-value");
                    const extendTimeRate = this.getAttribute("data-rate");

                    ShowLoading();

                    ExtendDeviceTime(deviceId, extendTime, extendTimeRate).then(
                        (result) => {
                            if (!result.success) {
                                showToast(
                                    result.message ||
        ...
        startButtonList.forEach((item) => {
            if (!item.classList.contains("event-attached")) {
                item.addEventListener("click", function () {
                    ...
                    //Rated time
                    if (timingType === "rated") {
                        StartRatedDeviceTime(deviceId).then((result) => {
                            if (!result.success) {
                                showToast(
                                    result.message ||
                                        "Possible network error occurred. Please see logs for more info.",
                    ...
                    //Open time
                        ...
                        StartOpenDeviceTime(deviceId).then((result) => {
                            if (!result.success) {
                                showToast(
                                    result.message ||
                                        "Possible network error occurred. Please see logs for more info.",
    ...
    $(document).on("click", ".confirm-end-time-button", function () {
        ...
        EndDeviceTimeManual(deviceId).then((result) => {
            if (!result.success) {
                showToast(
                    result.message ||
                        "Possible network error occurred. Please see logs for more info.",
