...
class DeviceTimeTransactionHandler {
    ...
    constructor(payload)
    {
        ...
        this.deviceRemainingTimerElement = getRemainingTimerElement(this.deviceId);
        ...
        this.deviceExtendMenuButtonElement = getExtendMenuButtonElement(this.deviceId);
        ...
        this.deviceStatusRibbonElement = getDeviceStatusRibbonElement(this.deviceId);
        this.deviceNotificationBannerElement = getDeviceNotificationBannerElement(this.deviceId);
        ...
        this.deviceRemTimeNotifValue = getRemainingTimeNotifValue(this.deviceCardElement);
    ...
    DoHeartbeatCheck()
    {
        ...
        GetDeviceHeartbeat(this.deviceId)
            .then((result) => {
                if (!result.success) {
    ...
    DoSession()
    {
        ...
        if (this.transactionType === window.TimeTransactionTypeEnum.END ||
    ...
    UpdateDeviceCardLabels(reset = false, isOpenTime = false) {
        ...
        if (reset)
        {
            ...
            if (this.deviceCardElement)
            {
                ...
                const content = this.deviceCardElement.querySelector('.content');  
                const extraContent = this.deviceCardElement.querySelector('.extra');  
        ...
        else
        {
            ...
            if (startTimeElement)
            {
                startTimeElement.textContent = `Start time: ${convertTo12HourFormat(formatTime(new Date(startTime)))}`;
            }
            ...
            if (isOpenTime === false)
            {
                if (endTimeElement)
                {
                    endTimeElement.textContent = `End time: ${convertTo12HourFormat(formatTime(new Date(endTime)))}`;
                }
        ...
        if (this.deviceTotalRateLblElement)
            {
                ...
                {
                    openTimeRunningRate = ((this.remainingTimeValue / 60) / this.deviceOpenTimeTimeElement) * this.deviceOpenTimeRateElement;
                }
                
                this.deviceTotalRateLblElement.textContent = "Total charge/rate: PHP " + parseFloat(openTimeRunningRate).toFixed(2);
            }
    }
    ...
    CheckAndNotifyRemainingTime(show = false)
    {
        if (show)
        {
            ...
            {
                if (this.remainingTimeValue <= this.deviceRemTimeNotifValue * 60) 
                {
                    const content = this.deviceCardElement.querySelector('.content');  
                    const extraContent = this.deviceCardElement.querySelector('.extra');  
                    ...
                    this.deviceRemainingTimerElement.classList.add('!font-bold');
                }
                else 
                {
                    const content = this.deviceCardElement.querySelector('.content');  
    ...
    DefineRemainingTime()
    {
        ...
        if (this.transactionType === window.TimeTransactionTypeEnum.RESUME)
        {
            if (this.isOpenTime)
            {
                ...
                return Math.floor(((new Date(Date.now() + (this.totalUsedTime * 1000)) - this.resumeTime)) / 1000);
            }
        }

        if (this.isOpenTime)
        {
            return Math.floor((new Date(Date.now() + 1000) - this.startTime) / 1000);
        }
        else
        {
